...
void FeatureTracking::trackFeatures(Mat &image){
	...
	if(!haveTrack && pastImageExists){							//if I don't have matched features to track, do:
		...
		matcher.match(initDescriptors,newDescriptors,matches);	//Match descriptors
		...
		for( int i = 0; i < newDescriptors.rows; i++ ){
		   if( matches[i].distance < 2*minDist ) goodMatches.push_back(matches[i]);
		  }
		cerr << "Good Matches: " << goodMatches.size() << endl;
		for (int i = 0; i < goodMatches.size(); i++){
			trackKeypoints[i] 	= newKeypoints[goodMatches[i].trainIdx];
		}
	...
	if(haveTrack && pastImageExists) {							//Track features in following images
